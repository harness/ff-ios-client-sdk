...
class FeatureRepositoryTest: XCTestCase {
  ...
  override func setUp() {
    ...
    sut = FeatureRepository(
      token: nil, cluster: "1", storageSource: mockCache, config: config, target: target)
  ...
  func testInitDefaultRepository() {
    ...
    // When
    let defaultRepo = FeatureRepository(
      token: token, cluster: "1", storageSource: mockCache, config: config, target: target)
  ...
  func testGetEvaluationByIdSuccessReplaceSuccess() {
    ...
    sut?.target.identifier = "success"
    let allKey = CfConstants.Persistance.features("c34fb8b9-9479-4e13-b4cc-d43c8f6b1a5d", "success")
      .value
    ...
    operation?.getEvaluationById(
      "boolTestFlagKey", target: "success",
    ...
    // When
    let evals: [Evaluation]? = try? operation!.storageSource.getValue(
      forKey: "c34fb8b9-9479-4e13-b4cc-d43c8f6b1a5d_success_features")
    for eval in evals! {
  ...
  func testGetEvaluationByIdCloudSuccess() {
    ...
    let key = CfConstants.Persistance.feature(
      "c34fb8b9-9479-4e13-b4cc-d43c8f6b1a5d", target, eval.flag
    ).value
    try? operation!.storageSource.saveValue(eval, key: key)

    // When
    operation?.getEvaluationById(
      eval.flag, target: target,
      onCompletion: { (result) in
        switch result {
        case .failure(let error):
          // Then
  ...
  func testGetEvaluationByIdCloudFailureCacheFailure() {
    ...
    // When
    operation?.getEvaluationById(
      key, target: "cloud_failure_cache_failure",
      onCompletion: { (result) in
        switch result {
        case .failure(let error):
          // Then
  ...
  func testGetEvaluationByIdCloudFailureCacheSuccess() {
    ...
    let key = CfConstants.Persistance.feature(
      "c34fb8b9-9479-4e13-b4cc-d43c8f6b1a5d", "cloud_failure_cache_failure", eval.flag
    ).value
    try? operation?.storageSource.saveValue(eval, key: key)

    // When
    operation?.getEvaluationById(
      eval.flag, target: "cloud_failure_cache_failure",
      onCompletion: { (result) in
        switch result {
        case .failure(let error):
          // Then
