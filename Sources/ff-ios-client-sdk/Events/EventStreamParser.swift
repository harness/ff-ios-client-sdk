  ...
  private func extractEventsFromBuffer() -> [String] {
    ...
    while let foundRange = searchFirstEventDelimiter(in: searchRange) {
      ...
      let dataChunk = dataBuffer.subdata(
        with: NSRange(
          location: searchRange.location, length: foundRange.location - searchRange.location)
      )
    ...
    dataBuffer.replaceBytes(
      in: NSRange(location: 0, length: searchRange.location), withBytes: nil, length: 0)
